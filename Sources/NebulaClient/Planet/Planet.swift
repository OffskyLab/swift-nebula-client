...
private struct PlanetConnection {
    var stellarClient: StellarClient
...
extension RoguePlanet {
    ...
    public func call<T: Decodable>(
        ...
        guard let data = try await call(method: method, arguments: arguments) else {
            throw NebulaClientError.fail(message: "No result from \(namespace).\(service).\(method)")
        }
...
extension RoguePlanet {
    ...
    private func connection(for namespace: String) async throws -> PlanetConnection {
        ...
        guard let stellarAddress = result.stellarAddress else {
            throw NebulaClientError.serviceNotFound(namespace: namespace)
        }

        let stellarClient = try await StellarClient.connect(to: stellarAddress)
        let conn = PlanetConnection(stellarClient: stellarClient, stellarAddress: stellarAddress)
        connections[namespace] = conn
        return conn
    }

    private func perform(body: CallBody, via client: StellarClient) async throws -> Data? {
        let matter = try Matter.make(type: .call, body: body)
        let replyMatter = try await client.request(matter: matter)
        let reply = try replyMatter.decodeBody(CallReplyBody.self)
        if let error = reply.error {
            throw NebulaClientError.fail(message: error)
        }
        return reply.result
    }
}
...
extension RoguePlanet {
    ...
    private func failover(
        ...
        let newClient = try await StellarClient.connect(to: nextAddress)
